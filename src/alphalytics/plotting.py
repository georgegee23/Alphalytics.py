...
def plot_spearman_rank(spearmanr_ts: pd.Series, window: int, ax = None):
    ...
    data.columns = ["Spearman Correlation", f"{window}-Period Rolling Mean"]
    ...
    ax2 = ax1.twinx()
    ax2.plot(data[f"{window}-Period Rolling Mean"], color='black')
    ax2.set_ylabel('Rolling IC', color='black')
    ax2.tick_params(axis='y', labelcolor='black')
    ax2.legend([f"{window}-Period Rolling Mean"], loc='upper right')

    # Center the right y-axis around 0
    data2_max = data[f"{window}-Period Rolling Mean"].max()
    data2_min = data[f"{window}-Period Rolling Mean"].min()
